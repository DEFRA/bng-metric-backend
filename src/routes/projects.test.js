...
describe('#getProjects', () => {
  ...
  test('Should scope the query to the org the token is signed in as', async () => {
  ...
  // Two devices, one user, two orgs: each list must follow its OWN token.
  test('Should give concurrent sessions their own org scope', async () => {
    const deviceA = listRequest(USER_001, {}, multiOrgClaims(REL_A))
    const deviceB = listRequest(USER_001, {}, multiOrgClaims(REL_B))

    await getProjects.handler(deviceA, {})
    await getProjects.handler(deviceB, {})

    expect(renderWhere(deviceA.drizzle._chain.where).params).toContain(REL_A)
    expect(renderWhere(deviceB.drizzle._chain.where).params).toContain(REL_B)
  })
})
...
describe('#createProject', () => {
  ...
  // BMD-936 (revised): the token wins, and the stored row is only a fallback.
  // Resolving from bng.users alone would stamp a project with whichever org
  // signed in last on ANY device, so a second concurrent session could silently
  // move where this user's new projects land.
  test('Should stamp the token context, not a stale stored one', async () => {
    const drizzle = createMockDrizzleInsert(newProject, [
      { relationshipId: 'rel-other-device', orgId: 'org-other-device' }
    ])
    const request = {
      drizzle,
      ...credsFor(USER_003, {
    ...
    // The token answered it, so no fallback query is issued.
