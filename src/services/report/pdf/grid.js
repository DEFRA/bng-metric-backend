 ...
 * the real OS basemap and the synthetic self-describing basemap the tests use
 * to prove registration offline. The test therefore exercises the production
 * path.
 */
...
export function gridFromWmtsCapabilities(xml, tileMatrixSetId) {
  ...
  if (!setBlock) {
    throw new Error(
      `TileMatrixSet "${tileMatrixSetId}" not found in capabilities`
    )
  }

  const matrices = [
    ...setBlock.matchAll(/<TileMatrix>([\s\S]*?)<\/TileMatrix>/g)
  ]
    .map((match) => match[1])
    .map((block) => ({
      identifier:
      ...
      topLeftCorner: (tagText(block, 'TopLeftCorner') ?? '')
        .trim()
        .split(/\s+/)
        .map(Number),
  ...
  if (matrices.length === 0) {
    throw new Error(
      `TileMatrixSet "${tileMatrixSetId}" declared no TileMatrix entries`
    )
  }
 ...
 * definition is published machine-readably by the OS NGD API – Tiles at
 * /maps/vector/ngd/ota/v1/tilematrixsets/27700, and it is the grid the
 * ngd-base tileset's EPSG:27700 tiles are cut to.
 *
...
export function tileSpanMetres(grid, z) {
  ...
  if (resolution === undefined) {
    throw new Error(
      `Zoom ${z} is outside this grid (0..${grid.resolutions.length - 1})`
    )
  }
 ...
 *
 * Derived from the grid and zoom, never read off a tile. In the spike this
 * interval was originally taken from the tile object the synthetic source
 * returned; a real OS tile carries no such property, so through the proxy the
 * overlay silently stopped drawing and the visual proof disabled itself
 * without failing. Both the synthetic basemap and the vector overlay must
 * compute it from the same two numbers, which is why it lives here.
 */
