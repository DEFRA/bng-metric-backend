 ...
 *
 * Hand-rolled for the same reason the synthetic PNG fixture is: the report
 * pipeline carries no dependency beyond pdfkit, and MVT is small enough to
 * honour that. The format is protobuf, but only four wire types and a dozen
 * field numbers of it — see the MVT 2.1 spec and vector_tile.proto. The
 * decoder was verified byte-for-byte against @mapbox/vector-tile on a live
 * 18-layer, 1,054-feature Ordnance Survey tile during the spike.
 *
 * The reader is what the report builder uses on real Ordnance Survey tiles.
 * The writer exists for the test fixtures, so decode is proven by round-trip
 * against encode rather than trusted by eye.
 *
...
function encodeFeature(feature, keyPool, valuePool) {
  ...
  for (const [key, value] of Object.entries(feature.properties ?? {})) {
    tags.push(
      poolIndex(keyPool, key),
      poolIndex(valuePool, JSON.stringify(value))
    )
  }
  if (tags.length > 0) {
...
function encodeLayer(layer) {
  ...
  for (const feature of layer.features) {
    writer.lengthDelimited(
      LAYER_FEATURE,
      encodeFeature(feature, keyPool, valuePool)
    )
  }
