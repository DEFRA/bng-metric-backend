 ...
 * MVT, so the vector path is provable offline end to end: decode,
 * tile-local-to-world conversion and the page transform. A misdecoded byte
 * or a misplaced tile visibly breaks the coincidence with the overlay.
 *
 * Layers: `GB_land` (a full-tile square, exercising the real style) and
 * `Graticule` (_symbol 0 minor / 1 major, styled only for fixtures — see
 * vector-style.js; a real OS tile never contains it).
 */
function syntheticVectorTile(grid, z, col, row) {
  const span = tileSpanMetres(grid, z)
  const resolution = grid.resolutions[z]
  const [tileMinX, tileMaxY] = tileTopLeft(grid, z, col, row)
  const tileMaxX = tileMinX + span
...
/**
 * The vector counterpart of the stub WMTS capabilities: an OGC API
 * TileMatrixSet JSON document with the same shape OS publishes at
 * /maps/vector/ngd/ota/v1/tilematrixsets/27700.
 */
const STUB_GB_EXTENT_METRES = 1_400_000

function stubTileMatrixSetJson(grid) {
  return {
    id: '27700',
    ...
    tileMatrices: grid.resolutions.map((resolution, z) => {
      const width =
        grid.matrixWidths?.[z] ??
        Math.ceil(STUB_GB_EXTENT_METRES / (resolution * grid.tileSize))
      return {
        id: String(z),
        cellSize: resolution,
