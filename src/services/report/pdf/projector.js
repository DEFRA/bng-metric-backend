/**
 * The world → page transform — the single source of alignment in the report.
 *
...
export function projectorFor(envelope, frame, { pad } = {}) {
  const padded = pad === undefined ? envelope : padEnvelopeInline(envelope, pad)
  return makeProjector(fitEnvelopeToFrame(padded, frame), frame)
}

// Kept inline (rather than importing envelope.js) so this module has no
