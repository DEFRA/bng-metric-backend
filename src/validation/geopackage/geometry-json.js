 ...
 * validation, sizing and persist — so readGeoPackage stringifies each one once,
 * at decode, and carries the result alongside the geometry object as a
 * `geometryJson` field (see readLayer). The extract functions copy that field
 * onto the geometry rows they build, so persist reads it too.
 *
 ...
 * fallback covers geometry carriers assembled outside readGeoPackage.
 *
