...
const { readBaselineGeoPackage } = await import('./geopackage.js')

// ---------------------------------------------------------------------------

describe('readBaselineGeoPackage feature decoding', () => {
  it('returns GeoJSON features per logical layer and only IGGI is missing from the template', async () => {
    await withTempGpkgFile(fullReadBuffer(), (filePath) => {
      const r = readBaselineGeoPackage(filePath)
...
describe('readBaselineGeoPackage error cases', () => {
  it('throws when a geometry blob has invalid GeoPackageBinary magic', async () => {
    ...
    await withTempGpkgFile(buf, (filePath) => {
      expect(() => readBaselineGeoPackage(filePath)).toThrow(/bad magic/i)
    })
  })
  ...
  it('throws when the envelope indicator is out of range', async () => {
    ...
    await withTempGpkgFile(buf, (filePath) => {
      expect(() => readBaselineGeoPackage(filePath)).toThrow(
        /envelope indicator/i
      )
    })
  })
  ...
  it('throws when the blob declares an unsupported SRS', async () => {
    ...
    await withTempGpkgFile(buf, (filePath) => {
      expect(() => readBaselineGeoPackage(filePath)).toThrow(/Unsupported SRID/)
    })
  })
  ...
  it('throws when the file does not exist', () => {
    expect(() =>
      readBaselineGeoPackage(join(tmpdir(), `missing-${Date.now()}.gpkg`))
    ).toThrow()
  })
})

describe('readBaselineGeoPackage layer resolution', () => {
