...
import { makePolygon } from './gpkg-db-blobs.js'
import { isGeometrySqliteColumnType } from '../../src/validation/geopackage/geopackage-internals-sqlite.js'
import {
  createLayerTableDDL,
  createSystemTables,
  quoteIdent
} from './gpkg-db-ddl.js'
import { baselineSchema } from './gpkg-db-fixtures.js'
...
function registerOneFeatureLayer(
  ...
  if (!layerDef) {
    throw new Error(
      `Test layer "${layerKey}" must exist in gpkg-template.schema.json`
    )
  }
